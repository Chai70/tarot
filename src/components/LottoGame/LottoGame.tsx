...
// 初始化红球和蓝球池
const initialRedBalls = Array.from({ length: 35 }, (_, i) => i + 1);
const initialBlueBalls = Array.from({ length: 12 }, (_, i) => i + 1);
...
interface RecordsManagerProps {
  ...
  totalPages: number;
  onClose: () => void;
  onEdit: (record: LottoRecord) => void;
  onDelete: (recordId: string) => void;
  onAddOfficial: (recordId: string, officialData: { date: string; redBalls: number[]; blueBalls: number[] }) => void;
  onNextPage: () => void;
  onPrevPage: () => void;
  calculateMatches: (record: LottoRecord) => { redMatches: number; blueMatches: number } | null;
}

interface EditRecordDialogProps {
  record: LottoRecord;
  onSave: (record: LottoRecord) => void;
  onClose: () => void;
}
  ...
  const handleSave = () => {
    ...
    if (redBalls.length === 5 && blueBalls.length === 2) {
      ...
      if (editForm.officialRedBalls && editForm.officialBlueBalls) {
        ...
        if (officialRed.length === 5 && officialBlue.length === 2) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal edit-modal">
        ...
        <div className="input-group">
          <label>官方红球 (5个数字):</label>
          <input
            type="text"
            placeholder="例: 2,8,15,28,33"
...
const LottoGame: React.FC = () => {
  ...
  const [memorizedRecords, setMemorizedRecords] = useState<LottoRecord[]>([]);
  const [showMemoryDialog, setShowMemoryDialog] = useState(false);
  const [showRecordsManager, setShowRecordsManager] = useState(false);
  const [currentPage, setCurrentPage] = useState(0);
  const [editingRecord, setEditingRecord] = useState<LottoRecord | null>(null);
  ...
  // 加载本地存储的记录
  useEffect(() => {
    const saved = localStorage.getItem('lotto-memorized-records');
    if (saved) {
  ...
  const initializeBallContainers = useCallback(() => {
    ...
    const redBalls = availableRed.map(number => ({
      ...
      x: Math.random() * 280, // 容器宽度
      y: Math.random() * 150, // 容器高度
  ...
  useEffect(() => {
    ...
    const animationInterval = setInterval(() => {
      setRedBallsInContainer(prev => 
        prev.map(ball => {
          let newX = ball.x + ball.vx * 3; // 增加速度
  ...
  const handleDrawRedBall = () => {
    if (animatingBall || drawnRed.length >= 5 || redBallsInContainer.length === 0) return;
    ...
    setTimeout(() => {
      ...
      setAvailableRed(prev => prev.filter(b => b !== selectedBall.number));
  ...
  const handleDrawBlueBall = () => {
    if (animatingBall || drawnBlue.length >= 2 || blueBallsInContainer.length === 0) return;
    ...
    setTimeout(() => {
      ...
      setAvailableBlue(prev => prev.filter(b => b !== selectedBall.number));
  ...
  const handleDrawAllRed = () => {
    if (animatingBall || drawnRed.length >= 5 || redBallsInContainer.length === 0) return;
    const remaining = Math.min(5 - drawnRed.length, redBallsInContainer.length);
  ...
  const handleDrawAllBlue = () => {
    if (animatingBall || drawnBlue.length >= 2 || blueBallsInContainer.length === 0) return;
    const remaining = Math.min(2 - drawnBlue.length, blueBallsInContainer.length);
  ...
  const handleMemorizeResult = () => {
    if (isDrawingFinished) {
      ...
      localStorage.setItem('lotto-memorized-records', JSON.stringify(updatedRecords));
  ...
  const handleEditRecord = (record: LottoRecord) => {
    setEditingRecord(record);
    setShowEditDialog(true);
  };

  const handleSaveEdit = (updatedRecord: LottoRecord) => {
    const updatedRecords = memorizedRecords.map(record => 
  ...
  const calculateMatches = (record: LottoRecord) => {
  ...
  const isDrawingFinished = drawnRed.length >= 5 && drawnBlue.length >= 2;
  ...
  const handleBlueBallSelect = (ball: number) => {
    ...
    } else if (selectedBlue.length < 2) {
      setSelectedBlue([...selectedBlue, ball].sort((a, b) => a - b));
    }
  };

  const handleConfirmSelection = () => {
    if (selectedRed.length === 5 && selectedBlue.length === 2) {
  ...
  const isSelectionComplete = selectedRed.length === 5 && selectedBlue.length === 2;
    ...
    <div className="lotto-game-container">
      ...
      <div className="draw-sections">
        <div className="red-section">
          <h3>红球区 ({drawnRed.length}/5)</h3>
          <div className="button-group">
            <button 
              className="draw-button red" 
              onClick={handleDrawRedBall} 
              disabled={drawnRed.length >= 5 || !!animatingBall}
            >
              摇一个红球
            </button>
            <button 
        ...
        <div className="blue-section">
          <h3>蓝球区 ({drawnBlue.length}/2)</h3>
          <div className="button-group">
            <button 
              className="draw-button blue" 
              onClick={handleDrawBlueBall} 
              disabled={drawnBlue.length >= 2 || !!animatingBall || isAnimating}
            >
              摇一个蓝球
            </button>
            <button 
      ...
      <div className="results-container">
        <div className="drawn-balls-container">
          ...
          <div className="balls-display">
            ...
            {Array.from({ length: 5 - drawnRed.length }).map((_, i) => (
        ...
        <div className="drawn-balls-container">
          ...
          <div className="balls-display">
            ...
            {Array.from({ length: 2 - drawnBlue.length }).map((_, i) => (
        ...
        <>
          <div className="manual-selection">
            <div className="selection-section">
              <h3>选择红球 ({selectedRed.length}/5)</h3>
              <div className="balls-grid red-grid">
                {initialRedBalls.map((ball) => (
                  <button
                    ...
                    disabled={!selectedRed.includes(ball) && selectedRed.length >= 5}
                  >
                    {ball}
                  </button>
                ))}
            ...
            <div className="selection-section">
              <h3>选择蓝球 ({selectedBlue.length}/2)</h3>
              <div className="balls-grid blue-grid">
                {initialBlueBalls.map((ball) => (
                  <button
                    ...
                    disabled={!selectedBlue.includes(ball) && selectedBlue.length >= 2}
                  >
                    {ball}
                  </button>
                ))}
